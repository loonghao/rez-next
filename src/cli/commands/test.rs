...
impl PackageTestRunner {
        ...
        for pattern in requested {
            if pattern.contains('*') {
                ...
                let regex = regex::Regex::new(&regex_pattern)
                    .map_err(|e| RezCoreError::CliError(format!("Invalid pattern '{}': {}", pattern, e)))?;
    ...
    pub fn run_test(&mut self, test_name: &str) -> RezCoreResult<i32> {
        ...
        let (status, output, error, exit_code) = if self.dry_run {
            (TestStatus::Skipped, "Dry run - test not executed".to_string(), None, 0)
        } else {
            // Mock test execution for now
            match test_name {
                "unit" => (TestStatus::Success, "All unit tests passed".to_string(), None, 0),
                "integration" => (TestStatus::Failed, "Integration test output".to_string(), 
                                Some("Test failed: assertion error".to_string()), 1),
                "performance" => (TestStatus::Success, "Performance tests completed".to_string(), None, 0),
                _ => (TestStatus::Error, "".to_string(), 
                     Some(format!("Unknown test: {}", test_name)), -1),
            }
        };
    ...
    pub fn print_summary(&self) {
        ...
        let passed = self.test_results.iter().filter(|r| r.status == TestStatus::Success).count();
        let failed = self.test_results.iter().filter(|r| r.status == TestStatus::Failed).count();
        let errors = self.test_results.iter().filter(|r| r.status == TestStatus::Error).count();
...
pub fn execute(args: TestArgs) -> RezCoreResult<()> {
    ...
    if args.inplace && (!args.extra_packages.is_empty() || args.paths.is_some() || args.no_local) {
        return Err(RezCoreError::CliError(
            "Cannot use --inplace in combination with --extra-packages/--paths/--no-local".to_string()
        ));
    }
