...
async fn search_async(args: SearchArgs) -> RezCoreResult<()> {
    ...
    println!("📚 Searching {} repositories...", repo_manager.repository_count());
...
async fn perform_search(repo_manager: &RepositoryManager, args: &SearchArgs) -> RezCoreResult<Vec<SearchResult>> {
...
fn evaluate_package_match(package: &Arc<Package>, args: &SearchArgs, repo_name: &str) -> Option<SearchResult> {
...
fn display_search_results(results: &[SearchResult], args: &SearchArgs) -> RezCoreResult<()> {
    ...
    match args.format.as_str() {
        ...
        _ => {
            eprintln!("Unknown format: {}. Available formats: table, json, detailed", args.format);
            Ok(())
        }
    }
}
...
fn display_table_format(results: &[SearchResult], args: &SearchArgs) -> RezCoreResult<()> {
    // Print header
    if args.verbose {
        println!("{:<20} {:<10} {:<15} {:<8} {:<20}", "NAME", "VERSION", "REPOSITORY", "SCORE", "MATCHES");
        println!("{}", "-".repeat(80));
    } else {
    ...
    // Print results
    for result in results {
        let version_str = result.package.version
            .as_ref()
            .map(|v| format!("{:?}", v))
            .unwrap_or_else(|| "unknown".to_string());
        
        if args.verbose {
            println!("{:<20} {:<10} {:<15} {:<8.1} {:<20}",
                result.package.name,
                version_str,
                result.repository,
                result.match_score,
            ...
            let description = result.package.description
                .as_ref()
                .map(|d| if d.len() > 37 { format!("{}...", &d[..37]) } else { d.clone() })
...
fn display_json_format(results: &[SearchResult]) -> RezCoreResult<()> {
    let json_results: Vec<serde_json::Value> = results.iter().map(|result| {
        serde_json::json!({
