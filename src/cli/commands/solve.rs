...
use rez_core_solver::{DependencyResolver, SolverConfig, SolverStats, ConflictStrategy};
use rez_core_solver::dependency_resolver::ResolutionResult;
use rez_core_repository::simple_repository::{SimpleRepository, RepositoryManager};
...
pub fn execute(args: SolveArgs) -> RezCoreResult<()> {
    ...
    rt.block_on(async {
        solve_async(args).await
    })
}

/// Async solve implementation
async fn solve_async(args: SolveArgs) -> RezCoreResult<()> {
    println!("Resolving dependencies for: {}", args.requirements.join(", "));
...
fn parse_requirements(requirement_strs: &[String]) -> RezCoreResult<Vec<Requirement>> {
    ...
    for req_str in requirement_strs {
        let requirement: Requirement = req_str.parse()
            .map_err(|e| RezCoreError::RequirementParse(format!("Failed to parse '{}': {}", req_str, e)))?;
...
fn display_resolution_result(
    ...
    match args.format.as_str() {
        ...
        _ => {
            eprintln!("Unknown format: {}. Available formats: summary, detailed, json", args.format);
            Ok(())
        }
    }
}
...
fn display_summary(result: &ResolutionResult, args: &SolveArgs) -> RezCoreResult<()> {
    ...
    if !result.resolved_packages.is_empty() {
        ...
        for resolved_pkg in &result.resolved_packages {
            ...
            let status = if resolved_pkg.requested { "requested" } else { "dependency" };
            println!("  {} {} ({})", resolved_pkg.package.name, version_str, status);
    ...
    if !result.failed_requirements.is_empty() {
        println!("\n❌ Failed requirements ({}):", result.failed_requirements.len());
    ...
    if !result.conflicts.is_empty() {
        ...
        for conflict in &result.conflicts {
            ...
            println!("    Conflicting requirements: {}", 
                conflict.conflicting_requirements.iter()
                    .map(|r| r.to_string())
                    .collect::<Vec<_>>()
                    .join(", "));
            if !conflict.source_packages.is_empty() {
                println!("    Source packages: {}", conflict.source_packages.join(", "));
            }
        }
    }
    ...
    if args.stats {
        ...
        println!("Conflicts encountered: {}", result.stats.conflicts_encountered);
