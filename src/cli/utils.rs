...
pub fn validate_package_name(name: &str) -> RezCoreResult<()> {
    ...
    if name.contains(' ') {
        return Err(RezCoreError::PackageParse("Package name cannot contain spaces".to_string()));
    }
...
pub fn parse_env_var(arg: &str) -> RezCoreResult<(String, String)> {
    if let Some(pos) = arg.find('=') {
        ...
        if key.is_empty() {
            return Err(RezCoreError::RequirementParse("Environment variable key cannot be empty".to_string()));
        }
        
        Ok((key, value))
    } else {
        Err(RezCoreError::RequirementParse(format!("Invalid environment variable format: '{}'. Expected KEY=VALUE", arg)))
    }
}
...
mod tests {
    ...
    fn test_parse_env_var() {
        assert_eq!(parse_env_var("KEY=value").unwrap(), ("KEY".to_string(), "value".to_string()));
        assert_eq!(parse_env_var("PATH=/usr/bin").unwrap(), ("PATH".to_string(), "/usr/bin".to_string()));
