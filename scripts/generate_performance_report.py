...
class PerformanceReportGenerator:
    ...
    def calculate_summary_stats(self, data: Dict) -> Dict:
        ...
        for module_name, module_data in data.items():
            ...
            if times:
                ...
                module_stats["performance_score"] = max(
                    0, 100 - (module_stats["avg_time_ns"] / 1000000)
                )  # Convert to ms
    ...
    def generate_markdown_report(
        self, data: Dict, baseline_data: Dict, stats: Dict
    ) -> str:
        ...
        for module_name in self.modules:
            if module_name in stats["modules"]:
                ...
                report.append(
                    f"- **Average Time**: {module_stats['avg_time_ns']/1000000:.2f}ms"
                )
                report.append(
                    f"- **Performance Score**: {module_stats['performance_score']:.1f}/100"
                )

                # Baseline comparison if available
                if baseline_data and module_name in baseline_data:
                    baseline_stats = self.calculate_module_baseline_comparison(
                        data.get(module_name, {}), baseline_data.get(module_name, {})
                    )
                    if baseline_stats:
                        change = baseline_stats.get("change_percent", 0)
                        if change > 5:
                            report.append(
                                f"- **vs Baseline**: 🔴 {change:+.1f}% regression"
                            )
                        elif change < -5:
                            report.append(
                                f"- **vs Baseline**: 🟢 {abs(change):.1f}% improvement"
                            )
                        else:
                            report.append(
                                f"- **vs Baseline**: ⚪ {change:+.1f}% (no significant change)"
                            )
        ...
        low_performing = [
            name
            for name, stats in stats["modules"].items()
    ...
    def calculate_module_baseline_comparison(
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, required=True, help="Directory to save reports"
    )
    parser.add_argument(
        "--baseline-dir", type=Path, help="Directory containing baseline results"
    )
    parser.add_argument(
        "--format",
    ...
    print(
        f"Summary: {stats['total_benchmarks']} benchmarks across {stats['total_modules']} modules"
    )
