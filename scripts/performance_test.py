...
def benchmark_version_parsing():
    ...
    for _ in range(10):  # Multiple runs for statistical significance
        _, duration = time_function(
            lambda: [rez_core.Version(v) for v in test_versions]
        )
...
def benchmark_version_sorting():
    ...
    version_strings = [
        f"1.{random.randint(0, 99)}.{random.randint(0, 99)}" for _ in range(1000)
    ]
...
def benchmark_memory_usage():
    ...
    try:
        ...
        print(
            f"After creating 10k versions: {after_creation:.1f} MB (+{after_creation - baseline_memory:.1f} MB)"
        )
        print(
            f"After sorting: {after_sorting:.1f} MB (+{after_sorting - after_creation:.1f} MB)"
        )
        print(
            f"Memory per version: {(after_creation - baseline_memory) * 1024 / len(versions):.1f} KB"
        )
