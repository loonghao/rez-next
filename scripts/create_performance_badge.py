...
class PerformanceBadgeGenerator:
    ...
    def generate_badge_data(self, data: Dict) -> Dict:
        ...
        met_targets = sum(1 for module, score in module_scores.items() 
                         if module in self.performance_targets and 
                         score >= self.performance_targets[module])
        total_targets = len([m for m in module_scores.keys() if m in self.performance_targets])
        ...
        if total_targets > 0:
            ...
            badges["targets"] = {
                ...
                "color": "brightgreen" if targets_percentage >= 80 else 
                        "yellow" if targets_percentage >= 60 else "red",
                "style": "flat-square"
            }
        ...
        return {
            ...
            "targets_met": f"{met_targets}/{total_targets}" if total_targets > 0 else "0/0",
...
def main():
    ...
    parser.add_argument("--output-file", type=Path, default="performance-badge.json",
                       help="Output file for badge data")
    parser.add_argument("--markdown-file", type=Path,
                       help="Output file for Markdown badges")
