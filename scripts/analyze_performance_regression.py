...
class PerformanceAnalyzer:
                    ...
                    for name, result in data.items():
                        if isinstance(result, dict):
                            results[name] = BenchmarkResult(
                                ...
                                throughput_ops_per_sec=result.get(
    ...
    def calculate_regression(
        ...
        change_percent = (
            (current.mean_time_ns - baseline.mean_time_ns) / baseline.mean_time_ns
        ) * 100
    ...
    def _calculate_confidence(
        ...
        baseline_cv = (
            baseline.std_dev_ns / baseline.mean_time_ns
            if baseline.mean_time_ns > 0
            else 1.0
        )
        current_cv = (
            current.std_dev_ns / current.mean_time_ns
            if current.mean_time_ns > 0
            else 1.0
        )
    ...
    def _create_details(
        self, baseline: BenchmarkResult, current: BenchmarkResult, change_percent: float
    ) -> str:
    ...
    def analyze_regressions(
        self,
        current_results: Dict[str, Dict[str, BenchmarkResult]],
        ...
        for module_name, current_module_results in current_results.items():
            ...
            for benchmark_name, current_result in current_module_results.items():
                if benchmark_name not in baseline_module_results:
                    print(
                        f"Warning: No baseline found for benchmark {module_name}::{benchmark_name}"
                    )
                    continue
                ...
                if (
                    regression
                    and abs(regression.change_percent) >= self.regression_threshold
                ):
    ...
    def generate_report(self, regressions: List[RegressionResult]) -> Dict:
        ...
        regressions.sort(
            key=lambda r: (severity_order[r.severity], abs(r.change_percent)),
            reverse=True,
        )
        ...
        for severity in RegressionSeverity:
            severity_counts[severity.value] = len(
                [r for r in regressions if r.severity == severity]
            )
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default="regression-analysis.json",
    ...
    if not baseline_results:
        ...
        if args.verbose:
            print(
                f"Found {len(regressions)} regressions above {args.threshold}% threshold"
            )
            for regression in regressions:
                print(
                    f"  {regression.benchmark}: {regression.change_percent:+.2f}% ({regression.severity.value})"
                )
    ...
    critical_regressions = len(
        [r for r in report["regressions"] if r["severity"] == "critical"]
    )
    if critical_regressions > 0:
        print(
            f"CRITICAL: {critical_regressions} critical performance regressions detected!"
        )
        sys.exit(1)
