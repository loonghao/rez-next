...
// ── Phase 136: Rex shell info messages output ─────────────────────────────────

/// rez rex: info messages appear in bash script output
#[test]
fn test_rex_info_messages_as_bash_comments() {
    use rez_next_rex::{RexEnvironment, ShellType, generate_shell_script};

    let mut env = RexEnvironment::new();
    env.info_messages.push("Activating myapp 1.0".to_string());
    env.info_messages.push("Setting up environment".to_string());

    let script = generate_shell_script(&env, &ShellType::Bash);
    assert!(!script.is_empty(), "Script must not be empty");
    assert!(script.contains("Activating myapp 1.0"),
        "Info message text should appear in script: {}", &script[..script.len().min(300)]);
}

/// rez rex: PowerShell script is non-empty even with only info messages
#[test]
fn test_rex_info_messages_powershell() {
    use rez_next_rex::{RexEnvironment, ShellType, generate_shell_script};

    let mut env = RexEnvironment::new();
    env.info_messages.push("Package activated".to_string());

    let script = generate_shell_script(&env, &ShellType::PowerShell);
    assert!(!script.is_empty(), "PS script with info messages should not be empty");
    // PS script should always have the generator header
    assert!(script.contains("Generated by rez-next rex"),
        "PS script should have generator header: {}", &script[..script.len().min(200)]);
}

/// rez rex: sourced script path appears in bash output
#[test]
fn test_rex_source_path_appears_in_bash() {
    use rez_next_rex::{RexEnvironment, ShellType, generate_shell_script};

    let mut env = RexEnvironment::new();
    env.sourced_scripts.push("/path/to/setup.sh".to_string());
    let script = generate_shell_script(&env, &ShellType::Bash);
    assert!(script.contains("setup.sh"),
        "Source path should appear in script: {}", &script[..script.len().min(300)]);
}

/// rez rex: multiple info messages appear in order
#[test]
fn test_rex_info_messages_order() {
    use rez_next_rex::{RexEnvironment, ShellType, generate_shell_script};
...
// ── Phase 137: EnvironmentSpec / ResolutionSummary ────────────────────────────

/// rez solver: ResolutionResult.to_environment_spec returns correct packages
#[test]
fn test_resolution_to_environment_spec_packages() {
    use rez_next_solver::resolution::ResolutionResult;
    use rez_next_package::Package;

    let mut pkg = Package::new("houdini".to_string());
    pkg.version = Some(rez_core::version::Version::parse("20.0").unwrap());
    pkg.tools = vec!["hython".to_string(), "houdini".to_string()];

    let result = ResolutionResult::new(vec![pkg]);
    let spec = result.to_environment_spec();
    assert_eq!(spec.packages.len(), 1);
    assert_eq!(spec.packages[0].name, "houdini");
    assert!(spec.packages[0].tools.contains(&"hython".to_string()));
}

/// rez solver: ResolutionSummary has correct package_count
#[test]
fn test_resolution_summary_package_count() {
    use rez_next_solver::resolution::ResolutionResult;
    use rez_next_package::Package;

    let pkgs: Vec<Package> = ["a", "b", "c", "d"].iter().map(|n| Package::new(n.to_string())).collect();
    let result = ResolutionResult::new(pkgs);
    let summary = result.get_summary();
    assert_eq!(summary.package_count, 4);
    assert!(!summary.conflicts_resolved);
    assert_eq!(summary.resolution_time_ms, 0);
}

/// rez solver: ResolutionResult.validate detects duplicate packages
#[test]
fn test_resolution_validate_duplicate_package() {
    use rez_next_solver::resolution::ResolutionResult;
    use rez_next_package::Package;

    let mut p1 = Package::new("python".to_string());
    p1.version = Some(rez_core::version::Version::parse("3.9.0").unwrap());
    let mut p2 = Package::new("python".to_string());
    p2.version = Some(rez_core::version::Version::parse("3.9.0").unwrap());

    let result = ResolutionResult::new(vec![p1, p2]);
    let validation = result.validate();
    assert!(validation.is_err(), "Duplicate packages should fail validation");
}

/// rez solver: ResolutionResult.validate passes for unique packages
#[test]
fn test_resolution_validate_unique_packages_ok() {
    use rez_next_solver::resolution::ResolutionResult;
    use rez_next_package::Package;

    let mut p1 = Package::new("python".to_string());
    p1.version = Some(rez_core::version::Version::parse("3.9.0").unwrap());
    let mut p2 = Package::new("maya".to_string());
    p2.version = Some(rez_core::version::Version::parse("2023.0").unwrap());

    let result = ResolutionResult::new(vec![p1, p2]);
    assert!(result.validate().is_ok(), "Unique packages should pass validation");
}

// ── Phase 138: DependencyGraph requirement/constraint/exclusion ───────────────

/// rez solver: DependencyGraph.add_requirement succeeds
#[test]
fn test_dependency_graph_add_requirement() {
    use rez_next_solver::DependencyGraph;
    use rez_next_package::PackageRequirement;

    let mut g = DependencyGraph::new();
    let req = PackageRequirement::parse("python>=3.9").unwrap();
    let result = g.add_requirement(req);
    assert!(result.is_ok(), "Adding requirement should succeed");
}

/// rez solver: DependencyGraph.add_exclusion increments exclusion_count
#[test]
fn test_dependency_graph_add_exclusion_count() {
    use rez_next_solver::DependencyGraph;

    let mut g = DependencyGraph::new();
    let result = g.add_exclusion("bad_package".to_string());
    assert!(result.is_ok(), "Adding exclusion should succeed");
    let stats = g.get_stats();
    assert_eq!(stats.exclusion_count, 1);
}

/// rez solver: DependencyGraph.detect_conflicts returns empty on clean graph
#[test]
fn test_dependency_graph_no_conflicts_clean_graph() {
    use rez_next_solver::DependencyGraph;
    use rez_next_package::Package;
...
/// rez solver: DependencyGraph.get_stats includes constraint_count
#[test]
fn test_dependency_graph_constraint_count_two() {
    use rez_next_solver::DependencyGraph;
    use rez_next_package::PackageRequirement;

    let mut g = DependencyGraph::new();
    g.add_constraint(PackageRequirement::parse("python>=3.9").unwrap()).unwrap();
    g.add_constraint(PackageRequirement::parse("maya>=2023").unwrap()).unwrap();

    let stats = g.get_stats();
    assert_eq!(stats.constraint_count, 2, "Should have 2 constraints");
}

// ── Phase 139: EnvironmentSpec tools and metadata ─────────────────────────────

/// rez solver: EnvironmentSpec.get_package_names lists all packages
#[test]
fn test_environment_spec_get_package_names() {
    use rez_next_solver::resolution::{EnvironmentSpec, PackageSpec};
    use std::collections::HashMap;

    let spec = EnvironmentSpec {
        packages: vec![
            PackageSpec { name: "python".to_string(), version: Some("3.9.0".to_string()), requirements: vec![], tools: vec![] },
            PackageSpec { name: "nuke".to_string(), version: Some("14.0".to_string()), requirements: vec![], tools: vec![] },
        ],
        environment_vars: HashMap::new(),
        metadata: HashMap::new(),
    };
    let names = spec.get_package_names();
    assert!(names.contains(&"python".to_string()));
    assert!(names.contains(&"nuke".to_string()));
}

/// rez solver: EnvironmentSpec.get_all_tools collects tools from all packages
#[test]
fn test_environment_spec_get_all_tools() {
    use rez_next_solver::resolution::{EnvironmentSpec, PackageSpec};
    use std::collections::HashMap;
    ...
    let spec = EnvironmentSpec {
        packages: vec![
            PackageSpec {
                ...
                version: None,
                requirements: vec![],
                tools: vec!["hython".to_string(), "houdini".to_string()],
            },
            PackageSpec {
                name: "nuke".to_string(),
                version: None,
                requirements: vec![],
                tools: vec!["nuke".to_string(), "nukex".to_string()],
            },
        ],
        environment_vars: HashMap::new(),
        metadata: HashMap::new(),
    };
    let tools = spec.get_all_tools();
    assert!(tools.contains(&"hython".to_string()));
    assert!(tools.contains(&"nuke".to_string()));
    assert!(tools.contains(&"nukex".to_string()));
    assert_eq!(tools.len(), 4);
}

// ── Phase 140: Rex ShellType + RexEnvironment operations ──────────────────────

/// rez rex: ShellType::parse is case-insensitive
#[test]
fn test_rex_shell_type_case_insensitive() {
    use rez_next_rex::ShellType;
...
/// rez rex: RexEnvironment vars update and delete
#[test]
fn test_rex_environment_env_var_update_delete() {
    use rez_next_rex::RexEnvironment;
...
#[test]
fn test_rex_zsh_same_output_as_bash() {
    use rez_next_rex::{RexEnvironment, ShellType, generate_shell_script};

    let mut env = RexEnvironment::new();
    env.vars.insert("PKG_ROOT".to_string(), "/opt/pkg".to_string());
...
#[test]
fn test_rex_empty_env_minimal_script_all_shells() {
    use rez_next_rex::{RexEnvironment, ShellType, generate_shell_script};

    let env = RexEnvironment::new();
    for shell in [ShellType::Bash, ShellType::Fish, ShellType::PowerShell] {
        let script = generate_shell_script(&env, &shell);
        assert!(!script.is_empty(), "Even empty env should produce non-empty script (header)");
        assert!(script.contains("Generated by rez-next rex"),
            "Script should have generator header for {:?}", shell);
    }
}
