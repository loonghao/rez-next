...
fn test_version_range_open_upper_contains_versions() {
    ...
    assert!(open_range.contains(&v_lo), "Open range should contain lower bound");
    assert!(open_range.contains(&v_hi), "Open range should contain high version");
    assert!(!open_range.contains(&v_below), "Open range should not contain version below");
}
...
fn test_requirement_weak_field_from_tilde_prefix() {
    ...
    let req = PackageRequirement::parse("~python").unwrap();
    assert!(req.weak, "Requirement starting with ~ should have weak=true");
    assert_eq!(req.name, "python");
...
fn test_rex_bash_alias_with_special_chars() {
    let mut env = RexEnvironment::new();
    env.aliases.insert("mypkg".to_string(), "/opt/my pkg/bin/run".to_string());
    let script = generate_shell_script(&env, &ShellType::Bash);
    // The alias should contain the path (possibly quoted)
    assert!(script.contains("mypkg"), "Alias name 'mypkg' should appear in bash script");
}
