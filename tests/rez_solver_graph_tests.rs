...
fn test_resolver_prefer_oldest_with_multiple_deps() {
    ...
    assert_eq!(ver, "1.0.0",
        "prefer_latest=false should pick lib-1.0.0 (oldest satisfying), got '{}'", ver);
}
...
fn test_resolver_prefer_oldest_respects_upper_bound() {
    ...
    assert_ne!(ver, "3.0.0", "lib-3.0.0 should be excluded by <3 upper bound");
    assert_eq!(ver, "1.0.0",
        "prefer_latest=false with range lib-1+<3 should pick 1.0.0, got '{}'", ver);
}
...
fn test_resolver_prefer_oldest_transitive() {
    ...
    let reqs: Vec<Requirement> = ["app-1+"]
        .iter()
        .map(|s| s.parse().unwrap())
        .collect();
    ...
    let app_ver = result.resolved_packages.iter()
        .find(|p| p.package.name == "app")
        .and_then(|p| p.package.version.as_ref())
        .map(|v| v.as_str())
        .unwrap_or("?");

    let lib_ver = result.resolved_packages.iter()
        .find(|p| p.package.name == "lib")
        .and_then(|p| p.package.version.as_ref())
        .map(|v| v.as_str())
        .unwrap_or("?");

    assert_eq!(app_ver, "1.0.0",
        "prefer_latest=false: app should pick 1.0.0, got '{}'", app_ver);
    assert_eq!(lib_ver, "1.0.0",
        "prefer_latest=false: lib should pick 1.0.0, got '{}'", lib_ver);
}
    ...
    match rt.block_on(resolver.resolve(reqs)) {
        Err(e) => {
            ...
            assert!(
                ...
                "error message should mention 'python' or 'Strict mode', got: '{}'", msg
            );
        }
        Ok(res) => {
...
fn test_solver_conflict_error_message_multiple_missing() {
    ...
    assert!(msg.contains("missing_x"), "error should mention missing_x, got: '{}'", msg);
    assert!(msg.contains("missing_y"), "error should mention missing_y, got: '{}'", msg);
    assert!(msg.contains("missing_z"), "error should mention missing_z, got: '{}'", msg);
}
...
fn test_solver_conflict_error_message_format_stable() {
    ...
    assert!(
        ...
        "error message must begin with 'Strict mode' prefix, got: '{}'", msg
    );
    assert!(
        msg.contains("stable_check_pkg"),
        "error message must mention the missing requirement 'stable_check_pkg', got: '{}'", msg
    );
}
