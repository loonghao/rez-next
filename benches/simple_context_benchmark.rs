...
fn bench_context_basic(c: &mut Criterion) {
    ...
    group.bench_function("create_context", |b| {
        ...
        b.iter(|| black_box(ResolvedContext::from_requirements(requirements.clone())));
    });
...
fn bench_shell_execution(c: &mut Criterion) {
    ...
    group.bench_function("simple_command", |b| {
        ...
        b.iter(|| black_box(executor.execute_sync("echo 'test'")));
    });
    ...
    group.bench_function("with_environment", |b| {
        ...
        b.iter(|| black_box(executor.execute_sync("echo $TEST_VAR")));
    });
...
fn bench_context_serialization(c: &mut Criterion) {
    ...
    let context = ResolvedContext::from_requirements(vec![PackageRequirement::new(
        "python".to_string(),
        Some(VersionRange::new("3.9+".to_string()).unwrap()),
    )]);
    ...
    for format in &[
        ...
        group.bench_with_input(
            ...
            format,
            |b, format| {
                b.iter(|| black_box(ContextSerialization::serialize(&context, *format)));
            },
        );
        ...
        group.bench_with_input(
            ...
            &(format, serialized),
            |b, (format, data)| {
                b.iter(|| black_box(ContextSerialization::deserialize(data, **format)));
            },
        );
    }
...
fn bench_context_caching(c: &mut Criterion) {
    ...
    group.bench_function("fingerprint_generation", |b| {
        ...
        b.iter(|| black_box(context.get_fingerprint()));
    });
...
criterion_main!(
    context_basic_tests,
    context_functionality_tests,
    context_scalability_tests
);
