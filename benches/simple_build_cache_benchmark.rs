...
use rez_core_cache::{
    ...
    AdaptiveTuner, UnifiedPerformanceMonitor
};
...
fn bench_build_basic(c: &mut Criterion) {
    ...
    group.bench_function("detect_build_system", |b| {
        ...
        b.iter(|| {
            black_box(BuildSystem::detect(&source_dir))
        });
    });
...
fn bench_build_configuration(c: &mut Criterion) {
    ...
    let configs = vec![
        ...
        ("single_threaded", BuildConfig {
            max_concurrent_builds: 1,
            verbosity: BuildVerbosity::Silent,
            ..Default::default()
        }),
        ("parallel", BuildConfig {
            max_concurrent_builds: 4,
            verbosity: BuildVerbosity::Normal,
            ..Default::default()
        }),
        ("high_performance", BuildConfig {
    ...
    for (name, config) in configs {
        group.bench_with_input(
            ...
            |b, config| {
                b.iter(|| {
                    black_box(BuildManager::with_config(config.clone()))
                });
            },
        );
    }
...
fn bench_build_requests(c: &mut Criterion) {
    ...
    group.bench_function("simple_build_request", |b| {
        let package = Package::new("simple-lib".to_string(), Version::new("1.0.0".to_string()).unwrap());
    ...
    group.bench_function("complex_build_request", |b| {
        let package = Package::new("complex-lib".to_string(), Version::new("2.1.0".to_string()).unwrap());
...
fn bench_cache_basic(c: &mut Criterion) {
    ...
    group.bench_function("create_performance_monitor", |b| {
        b.iter(|| {
            black_box(UnifiedPerformanceMonitor::new())
        });
    });
...
fn bench_cache_configurations(c: &mut Criterion) {
    ...
    for (name, config) in configs {
        group.bench_with_input(
            ...
            |b, config| {
                b.iter(|| {
                    black_box(IntelligentCacheManager::new(config.clone()))
                });
            },
        );
    }
...
fn bench_build_cache_scalability(c: &mut Criterion) {
    ...
    for package_count in &[1, 5, 10, 20] {
        let packages: Vec<Package> = (0..*package_count)
            .map(|i| Package::new(format!("package_{}", i), Version::new("1.0.0".to_string()).unwrap()))
            .collect();
...
fn bench_performance_validation(c: &mut Criterion) {
    ...
    group.bench_function("combined_performance", |b| {
        let package = Package::new("combined".to_string(), Version::new("1.0.0".to_string()).unwrap());
...
// Main entry point
criterion_main!(build_basic_tests, cache_basic_tests, build_cache_advanced_tests);
