...
## 2026-03-26 (Run 13): Fix CI --all-features failures on PR #64

### What was done
- **Updated PR [#64](https://github.com/loonghao/rez-next/pull/64)** with additional commit to fix all CI failures
- Root cause: `python-bindings` feature was defined as `python-bindings = []` (no pyo3 dep) in all crate Cargo.toml files, but source code had `#[cfg(feature = "python-bindings")] use pyo3::prelude::*;` blocks. When CI's `rust-actions-toolkit` reusable workflow ran with `--all-features`, the feature activated but pyo3 wasn't available → compilation failures in Clippy, Tests, Docs, Coverage
- **Fix**: Removed `python-bindings` feature from all 8 workspace crates + root Cargo.toml. The `#[cfg(feature = "python-bindings")]` blocks remain as dead code (feature doesn't exist → cfg evaluates to false)
- Fixed rustdoc `invalid_html_tags` warning in `resolved_context.rs` (`Arc<Package>` → `` `Arc<Package>` ``)
- Fixed bare URL warnings in `build.rs` doc comments (wrapped URLs in backticks)
- Removed `scripts/add-python-bindings-feature.ps1` (no longer needed)
- Cleaned up commented-out pyo3 references from all Cargo.toml and src/lib.rs
...
### Status
⏳ PR #64 updated and pushed. Awaiting CI completion. When CI passes, PR can be squash-merged to main. The auto-merge workflow handles bot PRs; this PR from loonghao needs manual merge or explicit auto-merge enablement.

### Pipeline Status
The full CI/CD pipeline is in place (aligned with clawup):
1. CI: `rust-actions-toolkit` reusable workflow (fmt, clippy, docs, tests on 5 platforms, security audit, coverage)
2. Release: `release-please` → creates release PR → on merge creates tag → dispatches `release.yml` → builds 8 platform targets
3. Install: `install.sh` (Linux/macOS) and `install.ps1` (Windows) with SHA256 verification
4. Auto-merge: Bot PRs (release-please, renovate, github-actions) auto-merge when CI passes

### Open PRs
- **PR #64**: Clippy + python-bindings fix (awaiting CI)
- **PR #63**: release-please auto-created release PR for v0.1.1 (blocked until #64 merges and main is green)
- **PR #60**: renovate dep update (memmap2)
