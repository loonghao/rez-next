...
"#;
    ...
    // ── Phase 120: stop() / error() boundary tests ───────────────────────────
    ...
    /// Actions after stop() are still applied (current implementation behaviour)
    #[test]
    fn test_actions_after_stop_are_still_applied() {
        let mut exec = RexExecutor::new();
        let commands = r#"
...
"#;
        ...
        // stop() sets the flag
        assert!(env.stopped);
        // Actions before stop are applied
        assert_eq!(
            env.vars.get("BEFORE_STOP"),
            Some(&"yes".to_string()),
            "BEFORE_STOP should be set"
        );
        // Current implementation does NOT abort on stop; actions after are still applied
        assert_eq!(
            env.vars.get("AFTER_STOP"),
            Some(&"yes".to_string()),
            "In current impl, actions after stop() continue to be applied"
        );
    }
