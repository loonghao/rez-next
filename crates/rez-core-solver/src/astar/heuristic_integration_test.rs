...
use super::{
    AStarSearch, AdaptiveHeuristic, CompositeHeuristic, DependencyHeuristic, HeuristicConfig,
    HeuristicFactory, Package, PackageRequirement, PackageSearchCriteria, RepositoryManager,
    SearchState, SolverConfig,
};
use std::sync::Arc;
...
async fn test_heuristic_integration() {
    ...
    let requirements = vec![PackageRequirement {
...
async fn test_heuristic_factory_integration() {
    ...
    let simple_requirements = vec![PackageRequirement {
    ...
    let simple_result = search_simple
        .search(simple_requirements, simple_heuristic_fn)
        .await;
    assert!(simple_result.is_ok());

    // Test complex complexity
    let mut search_complex = AStarSearch::new(repo_manager, config, Duration::from_secs(30), 1000);
    ...
    let complex_result = search_complex
        .search(complex_requirements, complex_heuristic_fn)
        .await;
...
async fn test_scenario_heuristics() {
    ...
    let fast_result = search_fast
        .search(requirements.clone(), fast_heuristic_fn)
        .await;
    ...
    let thorough_result = search_thorough
        .search(requirements.clone(), thorough_heuristic_fn)
        .await;
    assert!(thorough_result.is_ok());

    // Test conflict-heavy scenario
    let mut search_conflict = AStarSearch::new(repo_manager, config, Duration::from_secs(30), 1000);
    ...
    let conflict_result = search_conflict
        .search(requirements, conflict_heuristic_fn)
        .await;
...
async fn test_heuristic_performance_comparison() {
    ...
    let mut search_thorough = AStarSearch::new(repo_manager, config, Duration::from_secs(5), 100);
    ...
    let start_time = std::time::Instant::now();
    let thorough_result = search_thorough
        .search(requirements, thorough_heuristic_fn)
        .await;
...
fn test_heuristic_consistency() {
    let state = SearchState::new_initial(vec![PackageRequirement {
