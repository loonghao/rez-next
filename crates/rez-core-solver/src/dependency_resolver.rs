...
impl DependencyResolver {
    ...
    async fn resolve_recursive(&mut self, state: &mut ResolutionState) -> Result<Vec<ResolvedPackageInfo>, RezCoreError> {
        ...
        while let Some(requirement) = state.get_next_requirement() {
            ...
            for candidate in candidates {
                ...
                if let Ok(resolved_info) = self.try_resolve_with_candidate(state, &candidate, &requirement).await {
    ...
    async fn find_candidate_packages(&mut self, requirement: &Requirement) -> Result<Vec<Arc<Package>>, RezCoreError> {
        // Check cache first
        ...
        // Search repositories
        let packages = self.repository_manager.find_packages(&requirement.name).await?;
        
        // Cache the results
        self.package_cache.insert(requirement.name.clone(), packages.clone());
        
        Ok(self.filter_candidates(&packages, requirement))
    }
    
    /// Filter candidate packages based on version constraints
    fn filter_candidates(&self, packages: &[Arc<Package>], requirement: &Requirement) -> Vec<Arc<Package>> {
        packages
            .iter()
            .filter(|pkg| {
    ...
    async fn try_resolve_with_candidate(
        ...
        for dep_req_str in &candidate.requires {
            let dep_requirement: Requirement = dep_req_str.parse()
                .map_err(|e| RezCoreError::RequirementParse(format!("Invalid requirement '{}': {}", dep_req_str, e)))?;
...
impl ResolutionState {
    ...
    fn add_requirement(&mut self, requirement: Requirement) {
        let req_key = format!("{}:{}", requirement.name, requirement.version_constraint.as_ref().map(|v| format!("{:?}", v)).unwrap_or_default());
    ...
    fn find_satisfying_package(&self, requirement: &Requirement) -> Option<&ResolvedPackageInfo> {
        self.resolved_packages
            ...
            .find(|pkg| {
                ...
                pkg.package.version.as_ref().map_or(true, |v| requirement.is_satisfied_by(v))
            })
    }
    
    fn check_conflicts(&self, candidate: &Arc<Package>, requirement: &Requirement) -> Option<ResolutionConflict> {
        ...
        for existing in &self.resolved_packages {
            if existing.package.name == candidate.name {
                if let (Some(existing_version), Some(candidate_version)) = (&existing.package.version, &candidate.version) {
