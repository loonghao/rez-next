...
pub fn remove_package_version(
    ...
    if removed_paths.is_empty() {
        return Ok(PackageRemoveResult::default());
    }
...
mod tests {
    ...
    fn test_remove_nonexistent_version() {
        ...
        assert_eq!(result.unwrap().versions_removed, 0);
    }
    ...
    fn test_preview_removal() {
        ...
        let paths = preview_removal("previewpkg", Some("1.0.0"), &[tmp.path().to_path_buf()]);
    ...
    fn test_preview_removal_family() {
        ...
        let paths = preview_removal("familypkg", None, &[tmp.path().to_path_buf()]);
        assert_eq!(paths.len(), 1);
    }
}
