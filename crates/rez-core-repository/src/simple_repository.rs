...
pub trait PackageRepository {
    ...
    async fn get_package(&self, name: &str, version: Option<&str>) -> Result<Option<Arc<Package>>, RezCoreError>;
...
impl SimpleRepository {
    ...
    async fn load_package_from_path(&self, package_py_path: &Path) -> Result<Package, RezCoreError> {
...
impl PackageRepository for SimpleRepository {
    ...
    async fn get_package(&self, name: &str, version: Option<&str>) -> Result<Option<Arc<Package>>, RezCoreError> {
        ...
        if let Some(version_str) = version {
            let target_version = rez_core_version::Version::parse(version_str)?;
...
impl RepositoryManager {
    ...
    /// Get a specific package version
    pub async fn get_package(&self, name: &str, version: Option<&str>) -> Result<Option<Arc<Package>>, RezCoreError> {
        for repository in &self.repositories {
...
"#;
        ...
        fs::write(package_dir.join("package.py"), package_py_content).await.unwrap();
