...
impl EnvironmentManager {
    ...
    pub async fn generate_environment(&self, packages: &[Package]) -> Result<HashMap<String, String>, RezCoreError> {
    ...
    fn extract_package_env_definitions(
        ...
        if let Some(ref commands) = package.commands {
            let command_env_defs = self.parse_commands_for_env_vars(commands, &package.name, priority)?;
    ...
    pub fn generate_shell_script(&self, env_vars: &HashMap<String, String>) -> Result<String, RezCoreError> {
        ...
        match self.config.shell_type {
            ShellType::Bash => {
                ...
                for (name, value) in env_vars {
                    script.push_str(&format!("export {}=\"{}\"\n", name, self.escape_bash_value(value)));
                }
            }
            ShellType::Cmd => {
            ...
            ShellType::PowerShell => {
                ...
                for (name, value) in env_vars {
                    script.push_str(&format!("$env:{} = \"{}\"\n", name, self.escape_powershell_value(value)));
                }
            }
            ShellType::Zsh => {
            ...
            ShellType::Fish => {
                ...
                for (name, value) in env_vars {
                    script.push_str(&format!("set -x {} \"{}\"\n", name, self.escape_fish_value(value)));
                }
            }
        }
