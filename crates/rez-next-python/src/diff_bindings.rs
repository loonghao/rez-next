...
/// Each package list is a `Vec<Package>` as returned by `ResolvedContext.resolved_packages`.
pub fn compute_diff(old_packages: &[Package], new_packages: &[Package]) -> Vec<PyPackageDiff> {
    ...
    // Sort for deterministic output: added, removed, upgraded, downgraded, unchanged
    diffs.sort_by(|a, b| {
...
// ─── Rust unit tests ─────────────────────────────────────────────────────────
//
// Tests live in a separate file to keep this module under 1000 lines.
// See `diff_bindings_tests.rs`.
