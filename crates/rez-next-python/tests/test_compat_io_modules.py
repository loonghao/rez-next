...
class TestPipModule:
    ...
    def test_convert_pip_to_rez(self):
        ...
        pkg = pip.convert_pip_to_rez(
            "numpy", "1.25.0", requires=["python>=3.8"], description="Numerical Python"
        )
...
class TestCliModule:
    ...
    def test_cli_run_known_commands_fail_closed(self):
        import rez_next.cli as cli

        for cmd in ["env", "solve", "build", "search", "config", "self-test"]:
...
class TestEnvModule:
    ...
    def test_rez_env_unknown_pkg_fails_gracefully(self):
        ...
        env_obj = rez.RezEnv(["nonexistent_pkg_xyz_999"], paths=["/nonexistent/path_xyz"])
        assert env_obj is not None
    ...
    def test_get_activation_script_bash(self):
        result = rez.get_activation_script([], shell="bash", paths=["/nonexistent/path_xyz"])
        assert isinstance(result, str)
