...
class TestToDotBasic:
    ...
    def test_returns_string(self):
        pkg = self._make_pkg("python", "3.9.0")
        ctx = self._make_context([pkg])
        dot = ctx.to_dot()
...
class TestToDotWithDependencies:
    ...
    def test_single_dependency_edge(self):
        pkg_a = self._make_pkg("python", "3.9.0", ["numpy"])
        pkg_b = self._make_pkg("numpy", "1.24.0")
        ctx = self._make_context([pkg_a, pkg_b])
        dot = ctx.to_dot()
        assert '"python-3.9.0" -> "numpy-1.24.0"' in dot

    def test_multiple_dependencies(self):
        pkg_a = self._make_pkg("app", "1.0.0", ["liba", "libb>=1.0"])
        pkg_b = self._make_pkg("liba", "1.0.0")
        pkg_c = self._make_pkg("libb", "2.0.0")
        ctx = self._make_context([pkg_a, pkg_b, pkg_c])
        dot = ctx.to_dot()
    ...
    def test_requirement_version_specifier_stripped(self):
        ...
        pkg_a = self._make_pkg("app", "1.0.0", ["lib>=1.0"])
        pkg_b = self._make_pkg("lib", "1.5.0")
        ctx = self._make_context([pkg_a, pkg_b])
        dot = ctx.to_dot()
    ...
    def test_requirement_with_conflict_prefix(self):
        ...
        pkg_a = self._make_pkg("app", "1.0.0", ["!bad_pkg"])
        pkg_b = self._make_pkg("bad_pkg", "1.0.0")
        ctx = self._make_context([pkg_a, pkg_b])
        dot = ctx.to_dot()
