"""
Rez-compatible exception hierarchy.

This module provides the same exception classes as `rez.exceptions`,
with the exact same hierarchy so that code can catch exceptions
...
class ResourceContentError(ResourceError):
    ...
    def __init__(self, value=None, path=None, resource_key=None):
        ...
        if resource_key is not None:
            msg.append(f"resource type: {resource_key!r}")
        if path is not None:
            msg.append(f"{self.type_name}: {path}")
...
class BuildContextResolveError(BuildError):
    ...
    def __init__(self, context):
        ...
        msg = f"The build environment could not be resolved:\n{context.failure_description}"
        super().__init__(msg)
...
def convert_errors(from_, to, msg=None):
    """Context manager that converts exceptions from one type to another.

    Args:
        from_: Exception type to catch.
        to: Exception type to raise.
    ...
    if exc:
        info = f"{exc.__class__.__name__}: {str(exc)}"
        if msg:
            info = f"{msg}: {info}"
        raise to(info)
