"""
Utilities related to process/script execution.

Mirrors ``rez.utils.execution`` API.
"""
from __future__ import annotations

from contextlib import contextmanager
from enum import Enum
from typing import Any, Callable, Iterable, Iterator
import io
import os
import stat
import subprocess
import sys


@contextmanager
def add_sys_paths(paths: Iterable[str]) -> Iterator[None]:
    """Add to ``sys.path``, and revert on scope exit.

    Rez API: ``rez.utils.execution.add_sys_paths()``
    """
    original_syspath = sys.path[:]
    sys.path.extend(paths)
    try:
        yield
    finally:
        sys.path = original_syspath


class Popen(subprocess.Popen):
    """:class:`subprocess.Popen` wrapper.

    It fixes some issues encountered in Maya and Katana (and potentially other
    DCCs) and also forces the encoding to be utf-8 if ``text=True`` or
    ``universal_newlines=True`` is set without specifying the encoding.

    Rez API: ``rez.utils.execution.Popen``
    """
    def __init__(self, args, **kwargs) -> None:
        # Avoid python bug described here: https://bugs.python.org/issue3905
        if "stdin" not in kwargs:
            try:
                file_no = sys.stdin.fileno()
            except (AttributeError, io.UnsupportedOperation):
                file_no = None

            if file_no is None and sys.__stdin__ is not None:
                file_no = sys.__stdin__.fileno()

            if file_no not in (0, 1, 2):
                kwargs["stdin"] = subprocess.PIPE

        if kwargs.get("text") or kwargs.get("universal_newlines"):
            kwargs["text"] = True
            if "encoding" not in kwargs:
                kwargs["encoding"] = "utf-8"

        super().__init__(args, **kwargs)


class ExecutableScriptMode(Enum):
    """Which scripts to create with ``create_executable_script``.

    Rez API: ``rez.utils.execution.ExecutableScriptMode``
    """
    single = 1
    py = 2
    platform_specific = 3
    both = 4


def create_executable_script(
    filepath: str,
    body: str | Callable,
    program: str | None = None,
    py_script_mode: ExecutableScriptMode | None = None,
) -> list[str]:
    """Create an executable script.

    Args:
        filepath: File to create.
        body: Contents of the script.  If a callable, its code is used.
        program: Name of program to launch the script.  Default is ``'python'``.
        py_script_mode: What kind of script to create.  Defaults to
            ``rezconfig.create_executable_script_mode``.

    Returns:
        List of filepaths of created scripts.

    Rez API: ``rez.utils.execution.create_executable_script()``
    """
    from rez_next.config import config
    from rez_next.utils.platform_ import platform_

    program = program or "python"
    py_script_mode = py_script_mode or config.create_executable_script_mode

    is_forwarding_script_on_windows = (
        program == "_rez_fwd"
        and platform_.name == "windows"
        and filepath.lower().endswith(".cmd")
    )

    if callable(body):
        from rez_next.utils.sourcecode import SourceCode
        code = SourceCode(func=body)
        body = code.source

    if not body.endswith('\n'):
        body += '\n'

    script_filepaths = [filepath]
    if program == "python":
        script_filepaths = _get_python_script_files(
            filepath, py_script_mode, platform_.name
        )

    for current_filepath in script_filepaths:
        with open(current_filepath, 'w') as f:
            if is_forwarding_script_on_windows:
                f.write("@echo off\n")
                f.write("%s.exe %%~dpnx0 %%*\n" % program)
                f.write("goto :eof\n")
                f.write(":: YAML\n")
            else:
                f.write("#!/usr/bin/env %s\n" % program)
            f.write(body)

        if os.name == "posix":
            os.chmod(
                current_filepath,
                stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH
                | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH,
            )

    return script_filepaths


def _get_python_script_files(
    filepath: str,
    py_script_mode: ExecutableScriptMode,
    platform: str,
) -> list[str]:
    """Evaluate the ``py_script_mode`` for the requested filepath on the given platform."""
    script_filepaths: list[str] = []
    base_filepath, extension = os.path.splitext(filepath)
    has_py_ext = extension == ".py"
    is_windows = platform == "windows"

    if (
        py_script_mode == ExecutableScriptMode.single
        or py_script_mode == ExecutableScriptMode.both
        or (py_script_mode == ExecutableScriptMode.py and has_py_ext)
        or (py_script_mode == ExecutableScriptMode.platform_specific and not is_windows)
        or (py_script_mode == ExecutableScriptMode.platform_specific and is_windows and has_py_ext)
    ):
        script_filepaths.append(filepath)

    if (
        not has_py_ext
        and (
            py_script_mode == ExecutableScriptMode.both
            or py_script_mode == ExecutableScriptMode.py
            or (py_script_mode == ExecutableScriptMode.platform_specific and is_windows)
        )
    ):
        script_filepaths.append(base_filepath + ".py")

    return script_filepaths


def create_forwarding_script(
    filepath: str,
    module: str | tuple[str, str],
    func_name: str,
    *nargs,
    **kwargs,
) -> None:
    """Create a 'forwarding' script.

    A forwarding script is one that executes some arbitrary Rez function.
    This is used internally by Rez to dynamically create a script that uses
    Rez, even though the parent environment may not be configured to do so.

    Rez API: ``rez.utils.execution.create_forwarding_script()``
    """
    from rez_next.utils.platform_ import platform_

    if platform_.name == "windows" and \
            os.path.splitext(filepath)[-1].lower() != ".cmd":
        filepath += ".cmd"

    doc: dict[str, Any] = dict(
        module=module,
        func_name=func_name,
    )

    if nargs:
        doc["nargs"] = nargs
    if kwargs:
        doc["kwargs"] = kwargs

    from rez_next.utils.yaml import dump_yaml
    body = dump_yaml(doc)
    create_executable_script(filepath, body, "_rez_fwd")
