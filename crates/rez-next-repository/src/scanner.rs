...
mod tests {
    ...
    fn test_matches_pattern_question_mark() {
        let scanner = make_scanner_no_background();
        // ? matches exactly one character: package.p? matches package.py (y is one char)
        assert!(scanner.matches_pattern("package.py", "package.p?"));
        // package.?? matches package.py (two chars after dot)
        assert!(scanner.matches_pattern("package.py", "package.??"));
        // package.? does NOT match package.py (two chars after dot)
        assert!(!scanner.matches_pattern("package.py", "package.?"));
    }
    ...
    fn test_is_package_file_no_filename_returns_false() {
        let scanner = make_scanner_no_background();
        // A path with no file component (root or empty) returns false
    ...
    fn test_should_exclude_path_git_directory() {
        let scanner = make_scanner_no_background();
        // The default exclude pattern ".git/**" should match paths containing .git
    ...
    fn test_clear_cache_resets_size() {
        let scanner = make_scanner_no_background();
        // Manually insert into scan_cache
    ...
    fn test_cache_statistics_hit_rate_calculation() {
        let scanner = make_scanner_no_background();
        // Simulate 3 hits, 1 miss → hit_rate = 0.75
    ...
    fn test_cache_statistics_prefix_hit_rate_calculation() {
        let scanner = make_scanner_no_background();
        // 2 total (1 hit + 1 miss), 1 prefix hit
    ...
    fn test_normalize_path_removes_current_dir_components() {
        let scanner = make_scanner_no_background();
        // ./foo/./bar → foo/bar (relative fallback path)
        let result = scanner.normalize_path(Path::new("foo/./bar"));
        // canonicalize may fail for non-existent paths; fallback removes CurDir
    ...
    fn test_normalize_path_removes_parent_dir_components() {
        ...
        // Should not contain ParentDir after normalization
