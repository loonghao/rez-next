...
impl SimpleScanner {
    ...
    pub async fn scan_repository(
        &self,
        root_path: &PathBuf,
    ) -> Result<SimpleScanResult, Box<dyn std::error::Error>> {
    ...
    fn collect_directories_recursive<'a>(
        ...
        Box<
            dyn std::future::Future<Output = Result<Vec<PathBuf>, Box<dyn std::error::Error>>>
                + Send
                + 'a,
        >,
    > {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (config_name, config) in configs {
        ...
        for i in 1..=iterations {
            ...
            println!(
                "  Iteration {}: {}ms, {} packages, {} dirs, {} files",
                i,
                duration.as_millis(),
            ...
            println!(
                "    I/O: {}ms, Cache hits: {}, Memory mapped: {}",
                result.io_time_ms, result.cache_hits, result.memory_mapped_files
            );
        }
        ...
        println!(
            "📊 Average: {}ms, {:.1} packages/second",
            avg_duration, packages_per_second
        );
    }
